...
def extract_spike_features(time, current, voltage, start = 0.1, end = 0.7, fil = 10):
    ...
    这个文件被修改过，增加了如下信息，rebound，sag_time，sag_area计算了从最低刺激电流到0的所有值。原文件只是计算-200pA的最低电流
    """
    ...
    for c, curr in enumerate(current):
        ...
        EphysObject = efex.EphysSweepFeatureExtractor(t = time, v = voltage[:, c], i = current_array, start = start, \
                                                      end = end, filter = fil,use_filter=True)
        ...
        if c<8 and curr<=0 :  # rebound, sag_time, sag_area,只计算前面7个trace，后面的trace有时候未注射电流也会引起电压高于静息电位，从而出错
            ...
            # 这里有时出现这样的问题，标注的刺激电流为负数，但是刺激时段的电压却大于静息电位，不知哪里的问题，在这里，我们将这种sweep的sag均设为0         
            #判断条件是，刺激开始到结束的电压平均值大于静息电位
            if np.mean(voltage[start_index:end_index, c]) > v_baseline:
                warnings.warn('negative current cause upregulated voltage, recording data mistake ')
            else:
                if np.flatnonzero(voltage[end_index:, c] > v_baseline).size == 0: # So perfectly zero here means                                                                  # it did not reach it
                    rebound = 0
                else:
